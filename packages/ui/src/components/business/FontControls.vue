...
const fillPresets = [
  ...
  // Row 3 - amber/orange
  "#ffedd5", "#fed7aa", "#fdba74", "#f59e0b", "#ea580c",
  // Row 4 - green
  "#dcfce7", "#bbf7d0", "#86efac", "#22c55e", "#16a34a",
];
const fillPicker = ref<HTMLInputElement | null>(null);
function setFill(c: string) { fillColor.value = c; }
function openFillPicker() { fillPicker.value?.click(); }
function onFillPicked(e: Event) { fillColor.value = (e.target as HTMLInputElement).value; }
        ...
        <template #menu="{ close }">
          <div class="color-grid">
            <button
              ...
              :style="{ background: c }"
              :aria-label="c"
              @click="setFill(c); close()"
            />
          </div>
          <button type="button" class="other-color-btn" @click="openFillPicker(); close()">其他颜色</button>
...
.swatch.selected { outline: 2px solid #3b82f6; outline-offset: 0; }
.other-color-btn {
  display: inline-flex;
  align-items: center;
  width: 100%;
...
.other-color-btn:hover { background: #f1f5ff; }
.color-input-hidden { position: absolute; left: -9999px; width: 1px; height: 1px; }
</style>
