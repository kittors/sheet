...
export function expandSelectionByMerges(ctx: Context, sel: { r0: number; c0: number; r1: number; c1: number }) {
  const norm = () => {
    ...
    for (const m of ctx.sheet.merges) {
      ...
      if (rOverlap && cOverlap) {
        ...
        if (next.r0 !== cur.r0 || next.c0 !== cur.c0 || next.r1 !== cur.r1 || next.c1 !== cur.c1) {
          sel.r0 = next.r0; sel.c0 = next.c0; sel.r1 = next.r1; sel.c1 = next.c1
          changed = true
          break
        }
      }
